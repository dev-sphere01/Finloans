...
const EditRole = ({ role, onRoleUpdated, onClose }) => {
  const { success, error } = notificationService()
    ...
    if (role) {
      setFormData({
        ...
        permissions: role.permissions
          ? role.permissions.flatMap(p => p.actions.map(action => `${p.resource}.${action}`))
          : []
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Transform permissions from flat array to grouped structure
      ...
      const response = await roleService.update(role._id, payload)

      if (response) {
        setFormSuccess(response.message || 'Role updated successfully!')
      ...
      const msg = err.error || 'Failed to update role'
      setFormError(msg)
      error(msg)
    } finally {
      setLoading(false)
        ...
        <div>
          ...
          <div className="border rounded-lg p-4 max-h-96 overflow-y-auto border-gray-300 bg-gray-50">
            {modulesData.map((module) => (
              <div key={module.module} className="mb-2 last:mb-0">
                {/* Module Header */}
          ...
          {selectedPermissions.size > 0 && (
            <div className="mt-2 p-3 rounded-md bg-blue-50 border border-blue-200">
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {Array.from(selectedPermissions).map((permission,index) => (
                  <span
                    key={index}
